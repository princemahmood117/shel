...
const ArrowButton = ({ direction = "right", onClick, className = "" }) => {
  ...
  return (
    <button
      ...
      {/* Hover icon */}
      <span className="absolute inset-0 flex items-center justify-center w-full h-full text-white duration-400 
        -translate-x-full bg-[rgb(33,33,88)] group-hover:translate-x-0 ease">
        <Icon className="w-6 h-6" />
      </span>

      {/* Default icon */}
      <span className="absolute flex items-center justify-center w-full h-full text-gray-600 transition-all duration-400 
